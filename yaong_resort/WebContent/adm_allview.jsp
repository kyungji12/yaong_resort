...
<!DOCTYPE html>
	...
	* {
	...
	<div style="width:70%; margin: auto; text-align:center;">
			<h2 style="text-align:left;">현재 예약 상황(관리자용)</h2>
			...
			<% available = rset.getString(3); //vip룸
				...
				<td><a class='booking' href="?contentPage=d_02_booking2.jsp?bookdate=<%=rset.getString(1) %>&roomtype=1"><%=available %></a></td>
			<%} else { %>
				<td><a class='booking' href="?contentPage=adm_oneview.jsp?bookdate=<%=rset.getString(1) %>&roomtype=1"><%=available %></a></td>
			<%}
				available = rset.getString(4); //일반룸
				if (available.equals("예약가능")){%>
				<td><a class='booking' href="?contentPage=d_02_booking2.jsp?bookdate=<%=rset.getString(1) %>&roomtype=2"><%=available %></a></td>
			<%} else { %>	
				<td><a class='booking' href="?contentPage=adm_oneview.jsp?bookdate=<%=rset.getString(1) %>&roomtype=2"><%=available %></a></td>
			<%}
				available = rset.getString(5); //합리적인룸	
				if (available.equals("예약가능")){%>
				<td><a class='booking' href="?contentPage=d_02_booking2.jsp?bookdate=<%=rset.getString(1) %>&roomtype=3"><%=available %></a></td>
			<%} else { %>
				<td><a class='booking' href="?contentPage=adm_oneview.jsp?bookdate=<%=rset.getString(1) %>&roomtype=3"><%=available %></a></td>
				</tr>
			<%}
				};%>
			</table>
