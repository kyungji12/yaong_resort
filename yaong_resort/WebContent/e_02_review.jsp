	...
	<body>
		...
		<div style="width:70%; margin: auto; text-align:center;">
			...
			while(rset.next()){
				...
				out.println("<a class='title' href=?contentPage=e_02_review_view.jsp?id="+rset.getInt(1)+">"+rset.getString(3)+"</a>");
			...
			<table >
			<tr>
				<td style="border-color:#f1f1f1;text-align:right;"><button class="button" type="button" onclick="location.href='?contentPage=e_02_review_insert.jsp'">글쓰기</button></td>				
			</tr>
			</table>
			<!--페이지  -->
			<br>
			...
			<%if(firstNum != 1){%>
			  <a href="?contentPage=e_02_review.jsp?sendFirstLink=<%=firstPage_firstNum%>">&laquo;</a>
			 <%}%>
			 <%-- 10쪽씩만 보이도록 처리 --%>
		       <%
                   for(int i=firstNum; i<firstNum+10; i++){
                       ...
                       out.print("<td>");
                       out.print("<a href=?contentPage=e_02_review.jsp?sendFirstLink="+firstNum+"&from="+((i*10)-9)+"&cnt=10>"+i+"</a>");
                       out.print("</td>");
                   }
               %>
			   <%if((firstNum+9)*10 < totalLineCnt){%>
			  <a href="?contentPage=e_02_review.jsp?sendFirstLink=<%=lastPage_firstNum%>&from=<%=lastPage_firstNum*10-9%>&cnt=10">&raquo;</a>
				 <%}%>
			</div>			
		</div>
	</body>
